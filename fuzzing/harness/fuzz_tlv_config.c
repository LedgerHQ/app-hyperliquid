/*
 * Hyperliquid-specific grammar-aware TLV mutator.
 *
 * Mirrors the per-app fuzz_tlv_config.c pattern used by app-ethereum: a
 * table of `tlv_tag_info_t` grammars (one per TLV-carrying INS) feeds
 * `fuzz_tlv_dispatch_mutate()` so that ~50% of structured mutations emit
 * full TLV blobs whose tags are valid for the active parser.
 *
 * What is hyperliquid-specific: the lib_tlv parser used by every action
 * handler is DER-encoded (see lib_tlv/tlv_library.c get_der_value_as_uint32).
 * Tag and length values >= 0x80 must be emitted in long form
 * (0x80 | n followed by n big-endian bytes). The stock SDK
 * `tlv_custom_mutate()` in fuzzing/mock/tlv_mutator.c writes each tag as a
 * single byte ("Data[pos] = t->tag"), which short-form-encodes high tags
 * and makes the parser misinterpret them as DER length prefixes.
 *
 * Almost every hyperliquid content tag (0xd0, 0xda, 0xdb, 0xdd-0xed) is in
 * the high range, so we cannot reuse the SDK mutator as-is. This file
 * provides a replacement implementation of:
 *   - current_tlv_fuzz_config (the global the SDK dispatcher writes into)
 *   - tlv_custom_mutate()     (the structural mutator)
 * Both shadow their SDK counterparts at link time; CMakeLists.txt
 * deliberately omits ${LEDGER_FUZZ_TLV_MUTATOR_SOURCE} for this app.
 */
...
static size_t build_complete(uint8_t *Data, size_t MaxSize, unsigned int Seed)
{
    ...
    for (size_t i = 0; i < current_tlv_fuzz_config.num_tags; i++) {
        ...
        size_t                need
            = der_size(t->tag) + der_size((uint32_t) len) + len;
        if (pos + need > MaxSize) {
            break;
        }
...
/* INS_PROVIDE_ACTION_METADATA payload — parsed by src/action_metadata.c.
 * Min/max value lengths match the get_*_from_tlv_data helpers in
 * lib_tlv/tlv_library.c plus the explicit size checks in handle_*().
 *
 * NB: tag_info fields are uint8_t (defined in the SDK header). We store the
 * raw tag byte here; der_emit() handles the DER framing at write time.
 */
