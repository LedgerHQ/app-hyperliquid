...
static size_t build_limit_order_tlv(uint8_t *buf,
                                    uint32_t asset,
                                    bool is_buy,
                                    const char *limit_px,
                                    const char *sz) {
...
static size_t build_bulk_order(uint8_t *buf,
                               e_grouping grouping,
                               bool with_builder,
                               const uint8_t *builder_addr) {
    uint8_t order_tlv[256];
    size_t order_len = build_limit_order_tlv(order_tlv, TEST_ASSET_ID, true, "3000", "0.5");
    size_t offset = 0;
    ...
    if (with_builder) {
        ...
        tlv_append_bytes(builder_inner,
                         &builder_off,
                         TAG_BUILDER_ADDR,
                         builder_addr,
                         ADDRESS_LENGTH);
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown(test_parse_builder_wrong_address_size_fails,
                                        setup,
                                        teardown),
