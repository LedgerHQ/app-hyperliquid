#include <string.h>
#include "os_print.h"
#include "cx.h"
#include "write.h"
#include "format.h"
#include "eip712_abstraction.h"
#include "eip712_common.h"
#include "constants.h"

static bool compute_message_hash(const char *chain,
                                 const uint8_t user[ADDRESS_LENGTH],
                                 const char *abstraction,
                                 const uint64_t *nonce,
                                 uint8_t *out) {
    cx_sha3_t hash_ctx;
    // clang-format off
    const char encoded_type[] = "HyperliquidTransaction:UserSetAbstraction(string hyperliquidChain,address user,string abstraction,uint64 nonce)";
    // clang-format on
    uint8_t tmp[sizeof(*nonce)];

    if (cx_keccak_init_no_throw(&hash_ctx, 256) != CX_OK) {
        return false;
    }

    if (!eip712_encode_dyn_val(&hash_ctx, (uint8_t *) encoded_type, strlen(encoded_type))) {
        return false;
    }

    if (!eip712_encode_dyn_val(&hash_ctx, (uint8_t *) chain, strlen(chain))) {
        return false;
    }

    if (!eip712_encode_val(&hash_ctx, user, ADDRESS_LENGTH)) {
        return false;
    }

    if (!eip712_encode_dyn_val(&hash_ctx, (uint8_t *) abstraction, strlen(abstraction))) {
        return false;
    }

    write_u64_be(tmp, 0, *nonce);
    if (!eip712_encode_val(&hash_ctx, tmp, sizeof(tmp))) {
        return false;
    }

    if (cx_hash_final((cx_hash_t *) &hash_ctx, out) != CX_OK) {
        return false;
    }
    return true;
}

bool eip712_abstraction_hash(const uint64_t *chain_id,
                             const char *chain,
                             const uint8_t user[ADDRESS_LENGTH],
                             const char *abstraction,
                             const uint64_t *nonce,
                             uint8_t *domain_hash,
                             uint8_t *message_hash) {
    char tmp[20 + 1];

    if (abstraction == NULL) {
        return false;
    }

    PRINTF("======== computing EIP-712 hashes ========\n");
    format_u64(tmp, sizeof(tmp), *chain_id);
    PRINTF("chain_id = %s\n", tmp);
    PRINTF("chain = \"%s\"\n", chain);
    PRINTF("user = 0x%.*h\n", ADDRESS_LENGTH, user);
    PRINTF("abstraction = %s\n", abstraction);
    format_u64(tmp, sizeof(tmp), *nonce);
    PRINTF("nonce = %s\n", tmp);
    PRINTF("==========================================\n");

    // domain configuration
    const char name[] = "HyperliquidSignTransaction";
    const char version[] = "1";
    const uint8_t verifying_contract[ADDRESS_LENGTH] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    };

    if (!eip712_compute_domain_hash(name, version, chain_id, verifying_contract, domain_hash)) {
        return false;
    }
    if (!compute_message_hash(chain, user, abstraction, nonce, message_hash)) {
        return false;
    }
    return true;
}
